...
save_mod_path_prestem <- here::here(
  "sims", 
  "results", 
  "anchored_smallbias_corr.3_5x16"
)
...
anchored_MLHS <- nn_module(
  ...
  initialize = function() {
    ...
    self$global_bias <- nn_parameter(torch_zeros(1, device = dev_select(sim_params$use_cuda)))

    if (sim_params$use_cuda){
      self$det1 <- self$det1$cuda()
      self$det2 <- self$det2$cuda()
      self$det3 <- self$det3$cuda()
      self$det4 <- self$det4$cuda()
    }
  ...
  forward = function(x) {
    ...
    return(out_x - out_0 + self$global_bias)
  },
...
res <- lapply(
  ...
  function(X) {
    ...
    sim_hshoe_det(
      ...
      nn_model = anchored_MLHS,   # torch nn_module,
