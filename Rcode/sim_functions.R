...
sim_fcn_hshoe_linreg <- function(
  ...
  while (!stop_criteria_met){
    ...
    if (time_to_report){
      ...
      if (want_all_params){
        global_dropout_vec[row_ind] <- as_array(slhs_net$fc1$get_dropout_rates(type = "global"))
        marginal_dropout_mat[row_ind, ] <- as_array(slhs_net$fc1$get_dropout_rates(type = "marginal"))
        tau_vec[row_ind] <- as_array(slhs_net$fc1$tau)
        w_mu_mat[row_ind, ] <- as_array(slhs_net$fc1$compute_posterior_param()$post_weight_mu)
        w_var_mat[row_ind, ] <- as_array(slhs_net$fc1$compute_posterior_param()$post_weight_var)
        
        atilde_mu_mat[row_ind, ] <- as_array(slhs_net$fc1$atilde_mu)
        btilde_mu_mat[row_ind, ] <- as_array(slhs_net$fc1$btilde_mu)
        atilde_logvar_mat[row_ind, ] <- as_array(slhs_net$fc1$atilde_logvar)
        btilde_logvar_mat[row_ind, ] <- as_array(slhs_net$fc1$btilde_logvar)
        sa_mu_vec[row_ind] <- as_array(slhs_net$fc1$sa_mu)
        sa_logvar_vec[row_ind] <- as_array(slhs_net$fc1$sa_logvar)
        sb_mu_vec[row_ind] <- as_array(slhs_net$fc1$sb_mu)
        sb_logvar_vec[row_ind] <- as_array(slhs_net$fc1$sb_logvar)
      }
