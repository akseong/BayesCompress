...
BayesianLayerNJ <- nn_module(
  ...
  reset_parameters = function(init_weight, init_bias){
    ...
    self$z_mu <- torch_normal(1, 1e-2, size = self$z_mu$size())      # potential issue (if not considered leaf node anymore?)
    if (!is.null(init_weight)) {
      self$weight_mu <- torch_tensor(init_weight)
    } else {
      self$weight_mu <- torch_normal(0, stdv, size = self$weight_mu$size())
    }
    ...
    self$z_logvar <- torch_normal(-9, 1e-2)
    self$weight_logvar <- torch_normal(-9, 1e-2)
    self$bias_logvar <- torch_normal(-9, 1e-2)
  },
