...
sim_params <- list(
  "sim_name" = "hshoe, no global scale params, 2 layers 16 8, nobatching, fcnal data.  ",
  "seed" = 21681,
  "n_sims" = 1, 
...
res <- lapply(
  ...
  function(X) sim_hshoe(
    ...
    save_mod_path_stem = here::here("sims", 
                                    "results", 
                                    paste0("fcnl_hshoe_noglobalscale_", 
                                           sim_params$n_obs, "obs_", 
                                           sim_params$sim_seeds[1]
                                    ))
  )
)
