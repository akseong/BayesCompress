...
For first layer only; $\alpha_i < 0.05$ in color.  Might consider using some type of correction (e.g. Bonferroni would get rid of all spurious detections).  But overall fairly good performance!  2 orders of magnitude lower dropout parameter values for active covariates vs. nuisance variables.
...
For first layer only; $\alpha_i < 0.05$ in color.  Perfect.  %\geq 4$ orders of magnitude lower dropout parameter values for active covariates vs. nuisance variables.
