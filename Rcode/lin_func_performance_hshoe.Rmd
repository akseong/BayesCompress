...
while (epoch < train_epochs & abs(loss_diff) > convergence_crit){
  ...
  if(epoch %% 1000 == 0 & verbose){
    ...
    dropout_alphas <- exp(slhs_net$fc1$get_log_dropout_rates())
...
lin_simdat <- contents$lin_simdat
n_obs <- contents$n_obs
true_coefs <- contents$true_coefs


# investigating
log_dropout <- function(hslayer, type = "local"){
  ...
  if (type == "local"){
    logvar_sum <- hslayer$atilde_logvar + hslayer$btilde_logvar
  } else if (type == "global"){
    logvar_sum <- hslayer$sa_logvar + hslayer$sb_logvar
  } else if (type == "marginal"){
    logvar_sum <- hslayer$atilde_logvar + hslayer$btilde_logvar + hslayer$sa_logvar + hslayer$sb_logvar
  }
...
mykable(
  ...
  cap = "Wald-like posterior weights' (mean^2 / variance), in 8 rows"
)
...
Setting the dropout threshold to 0.05 __ad hoc__ for inclusion, we get a binary error rate of:

```{r DROPOUT, results="hold"}
# alphas <- log_dropout(hslayer = slhs_net$fc1)
# slhs_net$fc1$get_log_dropout_rates()

hskeep <- exp(log_dropout(hslayer = slhs_net$fc1)) < .05

cat("binary error as percentages (sums to 1): \n")
binary_err(est = as_array(hskeep), tru = true_coefs != 0)

cat("binary error rates (FP + TN = 1, FN + TP = 1): \n")
binary_err_rate(est = as_array(hskeep), tru = true_coefs != 0)
...
get_lm_stats <- function(simdat, include = NULL, alpha = 0.05){
  ...
  if (simdat$d_in > simdat$n){
    ...
    if (!is.null(include)){
      ...
      replace_inds <- sample(setdiff(1:last_cov_ind, include), size = length(add_inds))
      lm
...
lm_y <- as_array(lin_simdat$y)
lm_x <- as_array(lin_simdat$x)[, post_params_inclusion]
include <- which(post_params_inclusion)
