...
From what I can tell, the full formulation appears to be:

$$\begin{align}
  x &\sim N(x; f(W), \sigma^2)
  \\
...
1) the model for the data itself is never addressed in _LUW 2017_.  I feel like I'm missing something fundamental here --- none of the literature appears to address this.  

  - the only place that a distributional model for the data is mentioned is in the ELBO, $\mathcal{L}(\phi) = E_{q_\phi(W|z) q_\phi(z)} \left[ \log \color{red}{p(x|W, z)} \right] - E_{q_\phi(z)} \left[ KL\left( q_\phi(W|z) || p(W|z) \right) \right]-KL \left(q_\phi(z) || p(z) \right)$.
  - it seems like this likelihood term simply gets replaced by MSE in the scalar response case (which would comport with an assumption that $x \sim N(f(W), \sigma^2)$, at least up to a multiplier), or cross-entropy when the data/response are pictures/labels (again, corresponding to an assumed multinomial dist'n).  This makes sense to me, BUT what doesn't make sense is that 1) the variance of the response is not modeled anywhere, and 2) the multiplier probably matters a little, no?
...
Since $x$ depends on $z$ only through $W$, $p(x|W,z) = p(x|W)$.  Thus the first term, $E_{q_\phi(W|z) q_\phi(z)} \left[ \log p(x|W, z) \right]$ is the expectation of the log-likelihood $\log p(x|W)$ under the variational distribution.

In the scalar response case, where (__I THINK___) $p(x | W) = N(x; f(W), \sigma^2)$, we have

$$\begin{align}
  \log p(x|W)
  &=
    -\frac{1}{2} \log (2 \pi \sigma^2)
    -\frac{1}{2 \sigma^2} \left( x - f(W) \right)^2
  \\
\end{align}$$
...
An approximation to $KL \left(q_\phi(z) || p(z) \right)$ is taken from $Molchanov, Ashukha, Vetrov 2017$, through which we obtain:

$$KL \left(q_\phi(z) || p(z) \right) \approx \sum_i (k_1 \sigma (k_2 + k_3 \log(\alpha_i))) - 0.5 m (-\log \alpha_i) - k_3)$$
where $k_1, k_2, k_3$ are constants, $\sigma$ is the sigmoid function, and $m$ is the softplus function.  Importantly, note that this term depends only on the implied dropout rate $\alpha_i = \frac{\sigma_{z_i}^2}{\mu_{z_i}^2}$ --- which we identified as having a form similar to a Wald statistic.



### optimization

We rely on torch's automatic differentiation and gradient calculations to minimize the objective function.  In the scalar response case, LUW 2017 uses $MSE + KL/n$, which is __almost__ an affine function of the ELBO (i.e. optimal parameters would be basically the same).  The only difference is that $MSE$ differs from the likelihood term by a scalar multiplier of $\frac{1}{2 \sigma^2}$.  
