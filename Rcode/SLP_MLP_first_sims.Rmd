...
Since $p(x|W, z) = N(x; f(W), \sigma^2)$, $x$ depends on $z$ only through $W$, $p(x|W,z) = p(x|W)$.  Thus the first term, $E_{q_\phi(W|z) q_\phi(z)} \left[ \log p(x|W, z) \right]$ is the expectation of the log-likelihood $\log p(x|W)$ under the variational distribution.

In the scalar response case, 
...
$$KL \left(q_\phi(z) || p(z) \right) \approx \sum_i (k_1 \sigma (k_2 + k_3 \log(\alpha_i))) - 0.5 m (-log \alpha_i) - 0.5 (m(-loga \alpha _i ) - k_3)$$
where $k_1, k_2, k_3$ are constants, $\sigm$ is the sigmoid function, and $m$ is the softplus function.
...
The ELBO (scalar response case) is:
