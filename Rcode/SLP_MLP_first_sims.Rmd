...
1) the model for the data itself is never addressed in _LUW 2017_.  Letting $f(W)$ indicate the neural network and $x$ the response, is the response modeled as $x \sim N(f(W), \sigma^2)$?  Is there a prior on the error variance?  

2) the priors for many parameters in the variational distribution are left largely implicit --- in particular, the priors placed on $\mu_{z_i}, \alpha_i, \mu_{ij}, \sigma_{ij}^2$.  

  - The code uses draws from Normal and log-Normal distributions _to initialize_ $\mu_{z_i}, \alpha_i, \mu_{ij}, \sigma_{ij}^2$

Am I misunderstanding something fundamental here?  Is it unnecessary to provide priors for these, and unnecessary to have a distributional model for the response?  





From what I can tell, the full formulation appears to be (do ML people even think about the variance of the observations?  Seems to be completely unimportant / elided in their literature):
