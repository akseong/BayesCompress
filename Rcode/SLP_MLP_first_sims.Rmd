...
A few things are left implicit in _LUW 2017_ (maybe ML people all know this to be the common assumption?) --- in particular, the priors placed on $\sigma^2$, $\mu_{z_i}, \alpha_i, \\mu_{ij}, \sigma_{ij}^2$?  
...
$$\begin{align}
  ...
  p(\sigma^2) &\propto 1
  \\
  p(W) &= \prod_{ij} p(w_{ij})
  \\
  w_{ij} | z_{i} & \sim N \left(w_{ij} | 0, z_{i}^2 \right)
  \\
  p(z) &= \prod_i p(z_{i}) \propto |z_{i}|^{-1}
  \\
\end{align}$$
