...
slnj_mse_vec <- mse_stats(slnj_MSEs)
...
rownames(mlnj_tab) <- c("MLP", "Spike-Slab", "LASSO")
colnames(mlnj_tab) <- c("FP", "TP", "FN", "TN", "FWER")
...
#mlnj average MSE
mlnj_MSEs <- mlnj_metrics[, c(2,4)]
lasso_MSEs <- t(sapply(res, function(X) c(X$lasso$fit_mse, X$lasso$coef_mse)))
ss_MSEs <- t(sapply(res, function(X) c(X$ss$fit_mse, X$ss$coef_mse)))
mlnj_mse_vec <- mse_stats(mlnj_MSEs)
lasso_mse_vec <- mse_stats(lasso_MSEs)
ss_mse_vec <- mse_stats(ss_MSEs)
...
__FWER__ is somewhat concerning in both settings.  I'm not sure how concerned we are with FWER in this type of setting (100 nuisance variables, only 4 "true" covariates).
...
- __covariates__: identical to setting 1a)

- __response__:  

  - non-linear relationship between response $y$ and $x_1, x_2, x_3, x_4$: $$y = f_1(x_1) + f_2(x_2) + f_3(x_3) + f_4(x_4) + \epsilon$$ with $\epsilon \sim N(0,1)$.
...
fcn_df %>% 
  ...
  labs(
    title = "additive contribution of x1, x2, x3, x4 to response",
    subtitle = TeX("E(y) = f_1(x_1) + f_2(x_2) + f_3(x_3) + f_4(x_4)"),
    color = ""
  )
