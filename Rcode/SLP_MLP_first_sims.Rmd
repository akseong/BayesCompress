...
__How to differentiate ourselves from LUW 2017__:  
...
- $E_{q_\phi(z)} \left[\log \dfrac{1}{p(y|z)} \right]$ is the expected surprise of the data under our model *when $z$ is sampled under $q_\phi(z)$*.


So the ELBO, essentially, balances 1) the expected difference in surprise between the variational and prior distributions with 2) the expected surprise of the data, all when sampling $z$ under the variational distribution.  So maximizing the ELBO involves minimizing the surprise of the data by adjusting the approximate posterior, while keeping the approximate posterior to the prior.
...
```{r FUNCTIONget_restab}
get_restab <- function(res, tru, log_alpha_threshold = -3, method_name = "Bnn", verbose = TRUE){
  require(Matrix)
  ...
  method_log_alphas <- t(sapply(res, function(X) X[[1]]$log_dropout_alphas))
  method_include <- method_log_alphas < log_alpha_threshold
  
  method_binary_err <- t(apply(
    method_include, 1, 
    function(X) binary_err(est = X, tru = tru)
  ))
  method_binary_err_rates <- t(apply(
    method_include, 1,
    function(X) binary_err_rate(est = X, tru = tru)
  ))
  
  method_fwer <- mean(method_binary_err_rates[,1] != 0)
  
  # other metrics
  method_metrics <- t(sapply(res, function(X) X[[1]]$other_metrics))
  ...
  method_res <- colMeans(method_binary_err_rates)
  ...
  method_res <- c(method_res, "FWER" = method_fwer)
  ...
  rownames(restab) <- c(method_name, "Spike-Slab", "LASSO")
  colnames(restab) <- c("FP", "TP", "FN", "TN", "FWER")
...
get_mses <- function(res, method_name = "Bnn", true_coefs){
  
  mse_stats <- function(res_MSEs){
    c(
      "fit_MSE_avg" = mean(res_MSEs[, 1]),
      "fit_MSE_sd" = sd(res_MSEs[, 1]),
      "coef_MSE_avg" = mean(res_MSEs[, 2]),
      "coef_MSE_sd" = sd(res_MSEs[, 2])
    )
  }
  
  #mlnj average MSE
  method_metrics <- t(sapply(res, function(X) X[[1]]$other_metrics))
  method_MSEs <- method_metrics[, c(2,4)]
  lasso_MSEs <- t(sapply(res, function(X) c(X$lasso$fit_mse, X$lasso$coef_mse)))
  ss_MSEs <- t(sapply(res, function(X) c(X$ss$fit_mse, X$ss$coef_mse)))
  method_mse_vec <- mse_stats(method_MSEs)
  lasso_mse_vec <- mse_stats(lasso_MSEs)
  ss_mse_vec <- mse_stats(ss_MSEs)
  
  mse_tab <- rbind(
    method_name = method_mse_vec,
    "Spike-Slab" = ss_mse_vec,
    "LASSO" = lasso_mse_vec
  )
  return(mse_tab)
}
...
```{r SLP_RESULTS}
fpath <- here("Rcode", "results", "linear_sim_sig1_2.Rdata")
load(fpath)
...
fname2 <- "MLP_linear_sim_sig1_part2_2.Rdata"
...
```{r}
get_restab(res = mlnj_res, tru = tru, log_alpha_threshold = -7, method_name = "MLP", verbose = TRUE)
```
...
Good news is that in every trial, true covariates have the 3 lowest $\alpha_i$ values.  However, only a few are able to pick up on covariate $x_2$
...
1. tweak training parameters (`convergence_crit`, `max_epochs`)  

1. figure out how to set alpha threshold  

  - BH analog?
  - figure out a Bayesian FDR metric?

1. need better competitors for the functional data analysis (VCBART?  LASSO neural network?)  
