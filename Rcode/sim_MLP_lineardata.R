...
for(sim_num in 1:n_sims){
  ...
  while (epoch < max_train_epochs & !converge_stop & !loss_ma_stop){
    ...
    if(epoch %% 1000 == 0 & verbose){
      ...
      mlnj_bin_err <- binary_err(est = as_array(mlnj_keeps), tru = lin_simdat$true_coefs != 0 )
      cat("binary error: \n")
    ...
    if (epoch > burn_in) {
      ...
      log_alpha_mat[store_ind, ] <- c(
        ...
        mse$item(), 
        kl$item(),
        sum((mlnj_net$fc1$compute_posterior_param()$post_weight_mu - true_coefs)^2)$item()
      )
  ...
  if (verbose){
    ...
    mlnj_bin_err <- binary_err(est = mlnj_keeps, tru = lin_simdat$true_coefs != 0 )
