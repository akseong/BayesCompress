...
# change from torch_horseshoe_opus:
# 1) implements regularized horseshoe 
# 2) uses smaller variance for prior on bias terms; N(0, 0.01) by default
#    to resolve instability in predicted curves when x = 0
#    hopefully corrects instability at x=0
...
torch_reghs <- nn_module(
  ...
  initialize = function(
    ...
    bias_priorvar = 0.01,
    c_a = 2,  # ghosh & doshi-velez 2019
    c_b = 6,  # ghosh & doshi-velez 2019
    init_csq_mu = NULL,
    init_csq_logvar = NULL,
    init_alpha = NULL,
    init_weight = NULL, 
    init_bias = NULL, 
    init_sa = NULL, 
    ...
    self$z_type <- z_type
    self$bias_priorvar <- bias_priorvar
    self$c_a <- c_a
    self$c_b <- c_b
    ...
    # c^2 = slab parameter in regularized horseshoe
    self$csq_mu <- nn_parameter(torch_randn(1, device = self$devtype))
    self$csq_logvar <- nn_parameter(torch_randn(1, device = self$devtype))
    ...
    self$reset_parameters(
      init_alpha, init_weight, init_bias, init_sa, init_sb, init_atilde, init_btilde, init_csq_mu,
      init_weight_logvar, init_bias_logvar, init_sa_logvar, init_sb_logvar, init_atilde_logvar, init_btilde_logvar, init_csq_logvar
    )
  ...
  reset_parameters = function(
    ...
    # initialize means
    stdv <- sqrt(2 / self$weight_mu$size(2)) # Kaiming initialization.  changed to scale by # inputs.
    ...
    if (!is.null(init_csq_mu)) {
      self$csq_mu <- nn_parameter(torch_tensor(init_csq_mu, device = self$devtype))
    } else {
      self$csq_mu <- nn_parameter(torch_normal(4, 1, size = 1, device = self$devtype))
    }
    
    if (!is.null(init_csq_logvar)) {
      self$csq_logvar <- nn_parameter(torch_tensor(init_csq_logvar, device = self$devtype))
    } else {
      self$csq_logvar <- nn_parameter(torch_normal(4, 1, size = 1, device = self$devtype))
    }
  ...
  get_tau_mean = function(){
    mu_t  <- (self$sa_mu + self$sb_mu) / 2
    var_t <- (self$sa_logvar$exp() + self$sb_logvar$exp())/4
    torch_exp(mu_t + var_t / 2)
  },
  
  get_lambda_mean = function(){
    mu_l  <- (self$atilde_mu + self$btilde_mu) / 2
    var_l <- (self$atilde_logvar$exp() + self$btilde_logvar$exp())/4
    torch_exp(mu_l + var_l / 2)    
  },

  get_c_mean = function(){
    mu_c  <- (self$csq_mu) / 2
    var_c <- (self$csq_logvar$exp())/4
    torch_exp(mu_c + var_c / 2)
  },
  
  get_tau_mode = function(){
    mu_t  <- (self$sa_mu + self$sb_mu) / 2
    var_t <- (self$sa_logvar$exp() + self$sb_logvar$exp())/4
    torch_exp(mu_t - var_t)
  },
  
  get_lambda_mode = function(){
    mu_l  <- (self$atilde_mu + self$btilde_mu) / 2
    var_l <- (self$atilde_logvar$exp() + self$btilde_logvar$exp())/4
    torch_exp(mu_l - var_l)    
  },
  
  get_c_mode = function(){
    mu_c  <- (self$csq_mu) / 2
    var_c <- (self$csq_logvar$exp())/4
    torch_exp(mu_c - var_c)
  },
  ...
  forward = function(x){
    ...
    if (self$training){
      ...
      # Regularized horseshoe (and switching to regularized hshoe notation)
      log_lambda <- reparameterize(
        mu = 1/2 * (self$atilde_mu + self$btilde_mu),
        logvar = torch_log(self$atilde_logvar$exp() + self$btilde_logvar$exp()) - log(4),
        use_cuda = self$use_cuda
      )
      log_tau <- reparameterize(
        mu = 1/2 * (self$sa_mu + self$sb_mu),
        logvar = torch_log(self$sa_logvar$exp() + self$sb_logvar$exp()) - log(4),
        use_cuda = self$use_cuda
      )
      log_csq <- reparameterize(
        mu = self$csq_mu,
        logvar = self$csq_logvar
      ) 
      ...
      lambda_tilde_sq <- (csq*lambda$pow(2)) / (csq + lambda$pow(2)*tau$pow(2))
      z <- lambda_tilde_sq$pow(1/2) * tau
      ...
      if (self$z_type == "none") {
        ...
        if (self$z_type == "mean") {
          c <- self$get_c_mean()
          tau <- self$get_tau_mean()
          lambda <- self$get_lambda_mean()
        } else if (self$z_type == "mode") {
          c <- self$get_c_mode()
          tau <- self$get_tau_mode()
          lambda <- self$get_lambda_mode()
        }
        lambda_til <- c^2 * lambda^2 / (c^2 + tau^2*lambda^2)
        z = tau*lambda_til
  ...
  get_kl = function() {
    ...
    # KL(q(c^2) || p(c^2)); logNormal ||invGamma
    kl_csq <- KL_lognorm_IG(mu = self$csq_mu, logvar = self$csq_logvar, a = self$c_a, b = self$c_b )
    ...
    # KL for bias term
    kl_bias <- torch_sum(
      0.5 * (log(self$bias_priorvar) - self$bias_logvar +  (self$bias_logvar$exp() + self$bias_mu$pow(2))/self$bias_priorvar - 1)
    )
    
    # sum
    kl <- kl_csq + kl_sa + kl_sb + kl_atilde + kl_btilde + kl_w_z + kl_bias
    return(kl)
  }
)
