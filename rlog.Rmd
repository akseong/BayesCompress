...
# 2/9 1k update; possible data

## 1k obs sims results

Ran 5 sims for 1k training obs for 2 different sizes of networks (16-32 and 8-16 in hidden layers).  [Results](http://akseong.github.io/BayesCompress/Rcode/various_sim_results.html) are not good.  I'm guessing the network is just too big compared to the data causing major __overfitting:__ 
...
Furthermore, regarding network size, with 104 input covariates,
...
Going to try 1k obs trying a much smaller network and/or fewer nuisance covars.  
...
- I do kind of like that this happens, as it comports with expectations, and adds a reasonable calibration (network size) based on data size but not characteristics of data;  

- __WAIT__ what if we tried it again with a major __underestimate__ of $\tau_0$??  That might be interesting.  The current simulations still use an overestimate of the number of true variables (1 out of 10).
