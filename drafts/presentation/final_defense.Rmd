...
- excited to share work on Bayesian variable selection in semi-parametric models

- main interest: combine __BVS__ with __flexible models__ to analyze complex, nonlinear data, to figure out what matters and when.
...
- __Basic ideas__: 2 obstacles:
  - Misspecification: basis expansions commonly used to model functional data are misspecified for local testing
  - Multiplicity: properly specified prior over the model space $\rightarrow$ posterior probability of variable inclusion already accounts for multiplicity
...
+ quick (3 seconds) gambling game, ~ 200 rounds (10)
...
- **Electrocorticography (ECoG) data**
  - brain region activity ($y$) vs. covariates ($x$) at given time ($z$)
    - win/loss, risk, regret, expected earning
...
<img src="figs/thin_splfit.png" alt="spline basis example" style="width:auto; height:auto;">


???
- sine curve fit using (penalized) splines

- grey dotted lines are the individual basis functions

- data is projected onto basis functions

- colored bumps are basis functions multiplied by $\eta$.  Added up to produce the long curves.
...
???
__so local null testing might amount to testing whether the coefficients $\eta$ for bases covering a region are 0 or not.__
...
- if $\left( W^T W \right)^{-1} W^T$ is not sparse, (essentially, block diagonal across the regions) it is likely that the optimal coefficient values in $\eta_{1k}^\star$ associated with a specific region will be non-zero even when the group means are equal in that region.
...
- Interval-Wise Testing of Pini & Vantini 2017 --- distribution on area between curves.  Still runs into problem of asymptotic inconsistency.
...
???
- does not overlap, are orthogonal to each other
...
- paper describes theoretical results showing that the our method's posterior gamma concentrates around the optimal gamma
...
- comparison of Gaussian shrinkage prior and Intrinsic Conditionally Autoregressive (ICAR) prior with cut orthogonal splines, VC_BART, and the `gam` function from mgcv (additive model with generalized cross-validation; provides simultaneous 95% confidence bands; 12 and 24 knots)
...
???
ICAR prior - Intrinsic Conditional Autoregressive Prior (spatial prior) penalizing differences between neighboring regions
...
.panel[.panel-name[Win/Loss & Regret]

![](figs\ecog_sep_split1.JPG)

]

.panel[.panel-name[RPE & Gamble]
...
__Model effect of each covariate separately:__

- found increases in HFA activity associated with game wins, higher RPE, and regret

- not with the choice to gamble

__When all analyzed together;__ only regret has high inclusion probability
...
- seems innocuous / conventional / unthreatening, but __placing structure__ on $p(\lambda)$ $\rightarrow$ marginal prior dist'ns for $\beta$ with more __mass at 0 and in tails__ $\rightarrow$ bias posterior $\beta$'s to be either very small or large.  
...
???
shrinkage profile __under the prior__
...
- The goal is to minimize the (reverse) Kullback-Leibler Divergence (KL)
...
In order to construct $1-\kappa_j$ as a posterior inclusion probability, we need to recover an interpretation of $\tau^{(1)}$ as controlling the overall __input__ sparsity. 
...
- __call attention to $d_1$ in expression for $\tau$__
...
- also __general__, i.e. can apply to downstream _deterministic_ layers
...
we select acive covariates using the optimal BFDR control procedure of _Muller et al 2007_:
...
???
additionally for __scientific applications__ where different costs...
...
- our analysis does indicate a marginal effect < age 30 of being Hispanic


---
### Marginal effects as %: `college`, `goverment`, `self-employed`

<img src="figs/sal/marg_pct_wrk.png" alt="Your Description" style="width:100%; height:125%;">

- difference in real income __marginalized across all other covariates__

???

- supports previous finding of little effect of being self-employed



---
### Gender gap among salaried health workers:
...
???
- interestingly, college degree appears to close gender gap for Black males/females, but not for those identifying as non-Black.

- black healthcare workers with college degrees appear to earn about the same (~ $55 k) as non-Hispanic, non-Black males without college degrees
...
---
### Gender gap among salaried office workers:
...
- chose to look particularly at health and office workers because had some of the largest $n$ (for each subcategory) out of occupations

- trajectories unique within every subcategory

- college appears to lessen gender wage gap overall

- gender wage gap highest among non-Hispanic, non-Black. peaking around age 45

- gender wage gap smaller with college among everyone except non-Hispanic, non-Black

- also interesting to males across all categories tend to experience marked declines in wage around age 50, while women's wages in many groups are still climbing.  (this effect not nearly as prominent among health care workers)
...
- combine the flexible modeling and expressive power of deep neural networks with well-calibrated BFDR-control

- less risk due to misspecification of mean model

  - but also perhaps harder to answer specific questions via variable selection

- method leverages more data effectively without overfitting, exhibiting steady decreases in predictive error and increases in sensitivity

- develop corrections to inferential quantities that can be estimated both in purely Bayesian and partially Bayesian NNs


---
## Possible directions for future work

- combining horseshoe BNN with the cut basis

- joint scale parameter VI specification

- application to other types of models, e.g. VC model using sub-networks

- other methods for variable selection with neural networks

- theoretical support for corrections

???
- Ghosh and doshi-velez use low-rank matrix normal approximation for weights; we could use multivariate normal for scale parameters

- discrete spike-slab with concrete relaxation of Bernoulli; normalizing flow approximations for non-local priors

- theory: Polson Rockova pocterior conentraion for S-S RELU networks, Liu develops BvM theorems for BNN functionals
