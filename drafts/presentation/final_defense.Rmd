...
- can be rewritten as $w_{j,k} = \tilde{w}_{j,k} \lambda_j \tau$, where $\tilde{w} \sim N(0,1)$

--

$$w_{j,k} = \tilde{w}_{j, k} \sqrt{\alpha_{j} \beta_{j} a b}$$
...
- Also, shrinkage from the 2nd layer is arguably helpful, reducing overall dimension, providing sparser representations, mitigating identifiability issues.
...
Thus we propose correcting $\tau^{(1)}$ when composing $\kappa_j$ and using
