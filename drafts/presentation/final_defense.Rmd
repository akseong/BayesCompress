...
- $1 - \mathbb{E}[\kappa_j \mid \mathbf{y}]$ is analogous to BMA weight under discrete spike-and-slab

- Can be interpreted as a "pseudo"-Posterior Inclusion Probability ($PIP_j$) for variable $x_j$ (_Carvalho, Polson, Scott 2010_)
...
_Piironen & Vehtari 2017_ also use $\kappa_j$ to describe the __"effective number of coefficients"__ in a model:
$$m_\text{eff} = \sum_j (1 - \kappa_j)$$
...
because the marginal log likelihood of the data (also called the __model evidence__) is
...
- __IF TIME:__ one nice interpretation of the KL comes from information theory, where the negative log-density can be thought of the "information content", or how surprised we should be to see a random variable take on a particular value.  I.E. $\log\left(\frac{q_\phi(W)}{p(W|D)} \right)$ is how surprised we would be by a value of W arising from the posterior if we think that the actual distribution is $q_\phi(W)$.  Then we take the expectation of this surprise over $q_\phi(W)$. 
...
$$\begin{aligned}
  \tilde{w}_{j, k} & \sim N(0,1); &\{\mathbf W\} &= \left \{w_{i,j} = \tilde{w}_{j, k} \sqrt{\alpha_{j} \beta_{j} a b} \right \} 
  \\
...
- We have a __matrix__ of coefficients, with $d_\ell$ of them corresponding to a single $x_j$.  Each of the $d_l$ nodes captures __a different representation__ of the input variables.
--
Output of layer is also shrunk by horseshoe in next layer.
...
One way to estimate the effect of the __global__ sparsity induced by $\tau^{(1)}$ is to ignore the local shrinkage (set all $\lambda_j = 1$).  Then we might call

$$g_\text{eff} = \sum_{j=1}^p \sum_{k=1}^{d_1} \left( 1 - \frac{1}{1 + \tau^2} \right) = d_1 \times p \times \left( 1 - \frac{1}{1 + \tau^2} \right)$$
the _effective number of weights_ in $\mathbf{W}^{(1)}$ that are attributable to $\tau^{(1)}$
...
- 2nd layer provides __representation__ sparsity by shrinking layer 1 node outputs.
...
???
Now we have an estimate of the global sparsity.  How to separate input and representation sparsity?

recall the idea of the effective number of coefficients from Piironen & Vehtari 2017 $m_\text{eff} = \sum_j (1 - \kappa_j)$

---
### $m_\text{eff}^{(2)}$ correction

Thus we would like some correction $c$ such that $\tau^{(1)} \times c = \sqrt{\dfrac{g_\text{eff}}{m_\text{eff}^{(2)} \times p - g_\text{eff}}}$
...
- this problem (weak identifiability of $\tau$) is addressed by the 2nd correction

- since it's scalar, $\tau$ can move across the RELU activation with little impediment. 
...
- as a side-note, this should make it clear that we can view the shrinkage parameters $\lambda$ and $\tau$ as applying to the weights / coefs just as much as the inputs
...
- amplification of a $x_j$'s contribution to $\hat{y}$ by subsequent layers



???
Is it a $\tau$ correction?  Maybe not (amplification argument applies equally to $\lambda_j$ and $\tau$)  It's a scalar though, and it does help with the identifiability issue for $\tau$.  
- Plus I get to call it the SNM correction
...
- note that SS-GAM results at $n=5k$ are on 25 replicates b/c takes 4 hours to run and not very parallelizable.  
  - time complexity for most recent Spike-and-Slab implementations is O()
