...
kglobal50k_mat <- 
  ...
  ktc_metrictest_mat <- 
  ktc50k_mat <- matrix(NA, nrow = length(sim_fnames), ncol = ncol(sim_res$kappa_mat))
...
for (f_ind in 1:length(sim_fnames)){
  ...
  klocal50k_mat[f_ind, ] <- sim_res$kappa_local_mat[last_epoch, ]
  kglobal50k_mat[f_ind, ] <- sim_res$kappa_mat[last_epoch, ]
...
kmat_list <- list(
  "kglobal50k_mat" = kglobal50k_mat,
  "klocal50k_mat" = klocal50k_mat,
...
for (k in 1:nrow(ksn50k_mat)){
  ...
  err_mat_local <- metrics_err_by_max_bfdr(dropout_vec = klocal50k_mat[k, ], true_vec, bfdr_vec = max_bfdrs)
  klocal50k_bfdr_arr[k, , ] <- t(err_mat_local)
  
  err_mat_global <- metrics_err_by_max_bfdr(dropout_vec = kglobal50k_mat[k, ], true_vec, bfdr_vec = max_bfdrs)
  kglobal50k_bfdr_arr[k, , ] <- t(err_mat_global)
...
bfdr_arr_list <- list(
  ...
  "klocal50k_bfdr_arr" = klocal50k_bfdr_arr,
  "kglobal50k_bfdr_arr" = kglobal50k_bfdr_arr,
...
compiled_fname <- "C:/Users/ArnoldSeong/Github/Classes/BayesCompress/final_sims/compiled/hshoe4det1_5k_50sims_modfcns_bfdr_arr.Rdata"
load(compiled_fname)
# 
# 
# 
...
# roc1k
# ggsave(roc1k, filename = here("final_sims", "figs", "roc_hs2_1k.png"))
...
#   )
# bfdr_calibration
# ggsave(bfdr_calibration, filename = here("final_sims", "figs", "bfdr_calib_hs2_1k.png"))
