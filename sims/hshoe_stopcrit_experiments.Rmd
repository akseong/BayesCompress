...
# make this into "monotonic_k", i.e. able to check if increasing OR decreasing
increasing_k <- function(vec, inc_k = 100){
  ind = 1
  mono_inc <- FALSE
  while ((!mono_inc) & (ind <= (length(vec)-inc_k - 1))){
    mono_inc <- !any(diff(test_mse[ind:(ind+inc_k)]) < 0)
    ind <- ind + 1
  }
...
- tracking min test mse doesn't work very well - would hit stop criteria too soon
...
for (i in piece_num){
  ...
  ma50_increasing <- increasing_k(ma50, inc_k = 50)
...
## does test_mse escape certain window?
