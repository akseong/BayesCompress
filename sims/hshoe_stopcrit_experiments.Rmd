...
monotonic_k <- function(vec, inc_k = 100, increasing = TRUE){
  ...
  res <- ifelse(mono, c(ind, ind + k), FALSE)
  return(res)
}
...
- tracking min test mse and seeing if doesn't get any lower for $k$ epochs doesn't work very well - would hit stop criteria too soon
...
sd(test_mse[ind:(ind + sd_nobs)]) < diff(test_mse[ind + sd_nobs + 1:2])
...
portion_vec <- function(
  ...
  res <- rep(NA, length.out = n_portions)
  for (i in 1:n_portions){
    portion_vec <- vec[1:portion_size + portion_size * (i-1)]
    res[i] <- anal_fcn(portion_vec)
  }
  return(res)
}
