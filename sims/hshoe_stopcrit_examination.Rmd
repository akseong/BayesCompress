...
- only stopping criteria employed was max number of epochs (training epochs = 100k)

```{r LOAD_COMBINE_100k_SIMS}
...
# Mainline results: 100k training epochs
...
train_df_long %>%
  ...
  labs(
    title = "train_MSE by error type (70-100k epochs)"
  )
...
train_MA_df_long %>%
  ...
  labs(
    title = paste0(
      "moving average (",
      ma_k,
      ") train_MSE by error type (70-100k epochs)"
    )
  )
...
test_df_long %>%
  ...
  theme(legend.position = "none") + 
  labs(
    title = "test_MSE by error type (70-100k epochs)"
  )
...
test_MA_df_long %>%
  ...
  labs(
    title = paste0(
      "moving average (",
      ma_k,
      ") test_MSE by error type (70-100k epochs)"
    )
  )
```
...
tetr_df_long %>%
  ...
  theme(legend.position = "none") + 
  labs(
    title = "tetr_MSE by error type (70-100k epochs)"
  )
...
tetr_MA_df_long %>%
  ...
  labs(
    title = paste0(
      "moving average (",
      ma_k,
      ") tetr_MSE by error type (70-100k epochs)"
    )
  )
```
...
kl_df_long %>%
  ...
  theme(legend.position = "none") + 
  labs(
    title = "kl by error type (70-100k epochs)"
  )
...
kl_MA_df_long %>%
  ...
  labs(
    title = paste0(
      "moving average (",
      ma_k,
      ") kl by error type (70-100k epochs)"
    )
  )
...
sum(change_count_by_epoch[17:61, ])
sum(change_count_by_epoch)
```


Most crosses of the wald-stat threshold occur between epochs 18k - 61k (421 crossings out of a total of 460 crossings).
...
So these flips are all occurring in variable 1 (behavior to be expected).  Simulations 12, 23, 26, 54, 70, 72, 100
...
sum(alphas_60k[, 5:104] < wald_thresh)  # FP
sum(alphas_60k[, 1:4] > wald_thresh)    # FN
```
...
```{r}
errs_by_epoch <- matrix(NA, nrow = 100, ncol = 3)
colnames(errs_by_epoch) <- c("epoch", "FP", "FN")

for (epoch_row in 1:100){
...
err_rates_by_epoch[, 2] <- errs_by_epoch[, 2] / (100 * 100)
err_rates_by_epoch[, 3] <- errs_by_epoch[, 3] / (4 * 100)

mykable(err_rates_by_epoch[1:10 * 10, ])
```
