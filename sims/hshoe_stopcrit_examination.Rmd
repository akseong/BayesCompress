...
## Training & Stopping Criteria exploration
...
## test, train, test-train MSE increasing
...
kl_df_long %>%
  ...
  theme(legend.position = "none") + 
  labs(
    title = "kl_MSE by error type (70-100k epochs)"
  )
...
kl_MA_df_long %>%
  ...
  labs(
    title = paste0(
      "moving average (",
      ma_k,
      ") kl_MSE by error type (70-100k epochs)"
    )
  )
...
dim(above_w)
change_w <- apply(above_w, c(2, 3), function(X) abs(diff(X)))
change_count_by_epoch <- apply(change_w, c(1, 3), function(X) sum(X)
  )
